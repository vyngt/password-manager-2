use backend::business::usecases::{WriteToFileInput, WriteToFileUseCase};
...
async fn test_write_to_file_service_error() {
    ...
    match result.unwrap_err() {
        backend::errors::AppError::FileWriteError(msg) => {
...
async fn test_write_to_file_directory_not_found() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        backend::errors::AppError::DirectoryNotFoundError => {
            // Expected error
        }
