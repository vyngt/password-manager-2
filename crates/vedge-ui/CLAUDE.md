...
- No global state inside components. Cross-component communication goes through context at well-scoped boundaries — but **how** the context is provided matters:
  - **Per-instance context** (the component can be mounted more than once per page — Dialog, any compound Header/Body/Footer pattern): provide with an explicit **`<Provider value=…>`** element wrapping exactly the consuming subtree, **never** `provide_context()` in the `#[component]` body. Body-provided context leaks across sibling instances (last provider wins — every `DialogHeader` on a page resolved the *last-mounted* Dialog's `on_close`, so the ✕ closed the wrong dialog). For portaled components, the `<Provider>` goes *inside* the portal content. See `dialog.rs` for the shipped pattern and `docs/dialog_analyze.md` for the full post-mortem.
