...
- No global state inside components. Cross-component communication goes through context at well-scoped boundaries — but **how** the context is provided matters:
  - **Per-instance context** (the component can be mounted more than once per page — Dialog, any compound Header/Body/Footer pattern): provide with an explicit **`<Provider value=…>`** element wrapping exactly the consuming subtree, **never** `provide_context()` in the `#[component]` body. Body-provided context leaks across sibling instances (last provider wins — every `DialogHeader` on a page resolved the *last-mounted* Dialog's `on_close`, so the ✕ closed the wrong dialog). For portaled components, the `<Provider>` goes *inside* the portal content. See `dialog.rs` for the shipped pattern and `docs/technical/dialog_analyze.md` for the full post-mortem.
  - **App-level singletons** (Toast, ThemeState): body `provide_context` is fine — exactly one provider of the type exists, so there is no last-write-wins hazard. Provide once, near the root, unconditionally.
  - Litmus test when adding any `provide_context`: *"could two live instances exist in one page?"* Mounted counts, not open — every `<Dialog>` in a view is mounted even while closed.
