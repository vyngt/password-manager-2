use vedge_core::business::usecases::vault::{
...
pub async fn create_vault_item(
    ...
    state: State<'_, Mutex<AppState>>,
) -> Result<vedge_core::business::domain::entities::vault_item::VaultItem, ApiError> {
    let state = state.lock().await;
...
pub async fn list_vault_items(
    ...
    vedge_core::shared::pager::PaginationOutput<
        vedge_core::business::domain::entities::vault_item::VaultItem,
    >,
    ApiError,
> {
    let state = state.lock().await;
