...
fn full_unlock_flow_encrypt_then_decrypt_round_trips() {
    ...
    let input = *kdf.preprocess_2skd(master_password, &secret_key).unwrap();
    let master_key = kdf
        .derive_master_key(&input, &vault_salt, &fast_params())
        .unwrap();
    let kek = kdf.derive_kek(&master_key).unwrap();
    let verify_hash = kdf.derive_verify_hash(&master_key).unwrap();
    ...
    let plaintext = crypto
        .decrypt_entry(&dek_read, &nonce, &ciphertext, &aad)
        .unwrap();
...
fn wrong_password_produces_different_verify_hash() {
    ...
    let mk_ok = kdf
        ...
        .unwrap();
    let vh_ok = kdf.derive_verify_hash(&mk_ok).unwrap();

    let input_bad = *kdf.preprocess_2skd(b"WRONG password", &secret_key).unwrap();
    let mk_bad = kdf
        .derive_master_key(&input_bad, &vault_salt, &fast_params())
        .unwrap();
    let vh_bad = kdf.derive_verify_hash(&mk_bad).unwrap();
...
fn aad_substitution_fails_authentication() {
    ...
    let input = *kdf.preprocess_2skd(b"pw", &[0u8; SECRET_KEY_LEN]).unwrap();
    let mk = kdf
        .derive_master_key(&input, &[0u8; VAULT_SALT_LEN], &fast_params())
        .unwrap();
