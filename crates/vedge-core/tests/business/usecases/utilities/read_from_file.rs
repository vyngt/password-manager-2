use vedge_core::business::usecases::{ReadFromFileInput, ReadFromFileUseCase};
...
async fn test_read_from_file_service_error() {
    ...
    match result.unwrap_err() {
        vedge_core::errors::AppError::FileReadError(msg) => {
...
async fn test_read_from_file_not_found() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        vedge_core::errors::AppError::FileNotFoundError => {
            // Expected error
        }
