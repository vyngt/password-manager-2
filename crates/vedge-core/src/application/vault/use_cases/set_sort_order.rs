//! `set_sort_order` — set an entry's manual ordering position within its folder.
//!
//! `sort_order` lives inside the encrypted `CommonMeta` (like `is_favorite`), so
//! this mirrors `set_favorite` exactly: fetch existing, decrypt, mutate,
//! re-encrypt with a fresh nonce + bumped version, persist, update index, audit.
//! Crucially it never returns plaintext to the caller — a drag-reorder must not
//! reveal a secret-bearing entry's payload just to bump an integer.
...
#[instrument(skip_all, fields(entry_id = %entry_id, sort_order = order))]
pub async fn set_sort_order(
    ...
    if matches!(payload, EntryPayload::Unknown(_)) {
        return Err(VaultError::UnsupportedEntryType(
            "cannot reorder an unknown entry".into(),
        ));
    }
    payload.meta_mut().sort_order = order;
