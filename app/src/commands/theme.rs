use backend::business::usecases::theme::{
...
) -> Result<backend::business::domain::entities::theme::Theme, ApiError> {
...
pub async fn create_color_scheme(
    ...
    state: State<'_, Mutex<AppState>>,
) -> Result<backend::business::domain::entities::theme::ColorScheme, ApiError> {
    let state = state.lock().await;
...
pub async fn list_color_schemes(
    ...
    backend::shared::pager::PaginationOutput<
        backend::business::domain::entities::theme::ColorScheme,
    >,
    ApiError,
> {
    let state = state.lock().await;
